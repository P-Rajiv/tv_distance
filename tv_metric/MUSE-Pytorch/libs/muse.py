...
class MUSE(object):
    ...
    def generate(self, config, _n_samples, nnet, decode_fn, **kwargs):
        ...
        for step in range(_sample_steps):
            ...
            max_len = max(sampled_ids.size(1), ids.size(1))

            # Pad tensors to the maximum length
            sampled_ids = torch.nn.functional.pad(sampled_ids, (0, max_len - sampled_ids.size(1)))
            ids = torch.nn.functional.pad(ids, (0, max_len - ids.size(1)))
            is_mask = torch.nn.functional.pad(is_mask, (0, max_len - is_mask.size(1)))
            sampled_ids = torch.where(is_mask, sampled_ids, ids)
            sampled_logits = torch.nn.functional.pad(sampled_logits, (0, max_len - sampled_logits.size(1)))
