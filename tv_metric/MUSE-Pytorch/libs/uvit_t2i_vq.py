...
class UViT(nn.Module):
    def __init__(self, img_size=16, in_chans=8, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4.,
        ...
        self.num_vis_tokens = int((img_size) ** 2) + 1  # Adjust to add an extra token
        ...
        self.mlm_layer = MlmLayer(feat_emb_dim=embed_dim, word_emb_dim=embed_dim, vocab_size=2026)  # Match the checkpoint's vocab size
    ...
    def forward(self, masked_ids, context):
        ...
        if isinstance(context, str):
            context = torch.tensor([ord(c) for c in context], dtype=torch.float32, device= 'cuda').unsqueeze(0)
            # convert it to bert embedding of 768 dim
            context = self.context_embed(context)

        context_token = self.context_embed(context.type_as(x))
        context_token = context_token.expand(x.size(0), -1, -1)  
