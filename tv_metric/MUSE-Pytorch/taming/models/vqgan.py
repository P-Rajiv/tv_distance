...
class VQModel(nn.Module):
    ...
    def init_from_ckpt(self, path, ignore_keys=list()):
        sd = torch.load('pre_trained_models/vqgan_jax_strongaug.ckpt', map_location="cpu")
    ...
    def decode_code(self, code_b):
        code_b = torch.clamp(code_b, 0, self.embed_dim - 1)
...
def get_model(config_file='vq-f16-jax.yaml'):
    ...
    config = OmegaConf.load(f'configs/vae_configs/vq-f16-jax.yaml').model
